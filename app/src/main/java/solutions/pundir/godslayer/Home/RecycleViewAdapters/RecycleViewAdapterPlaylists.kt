...
class RecycleViewAdapterPlaylists internal constructor(context: Context?, val items: MutableList<DatabaseRecord>, val parent_fragment : FargmentHomePlaylists) : RecyclerView.Adapter<RecycleViewAdapterPlaylists.HomeItemViewHolder>() {
    ...
    override fun onBindViewHolder(holder: HomeItemViewHolder, position: Int) {
        ...
        val url = items[position].image_url.trim()
        if (url != "") {
            Picasso.get().load(url).fit().into(holder.recyclerViewHomeItemImage)
